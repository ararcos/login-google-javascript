...
from ..entities import SeatBookingResponse, SeatBooking
...
class BookingService:
    ...
    def create_booking(self, booking: SeatBooking) -> SeatBooking:
        ...
        if user_bookings:
            office_of_conflict = self.office_repository.get(user_bookings[0].office_id)
            raise UserAlreadyHasReservation(
                f"You already booked a desk this date ({booking.booked_date_init.date()})"
                f" at {office_of_conflict.name.capitalize()} office",
    ...
    def create_bookings(
        self, bookings: typing.List[SeatBooking]
    ...
    def find_bookings(
        self, criteria: Criteria, populate: bool
    ) -> typing.List[SeatBookingResponse]:
        bookings = self.booking_repository.find(criteria)
        if populate:
            bookings_response = []
            for booking in bookings:
                booking_response = SeatBookingResponse(**booking.__dict__)
                booking_response.seat = self.populate_seat(booking.seat_id)
                booking_response.office = self.populate_office(booking.office_id)
                bookings_response.append(booking_response)
            return bookings_response
        return bookings

    def update_booking(
        self, booking_id: str, user_id: str, booking: SeatBooking
    ) -> SeatBooking:
    ...
    def user_reservations_by_dates(
        ...
        return self.find_bookings(
            criteria=criteria_to_find_reservations, populate=False
        )
    ...
    def populate_seat(self, seat_id: str):
        seat = self.seat_repository.get(seat_id)
        if not seat:
            return None
        return seat.__dict__

    def populate_office(self, office_id: str):
        office = self.office_repository.get(office_id)
        if not office:
            return None
